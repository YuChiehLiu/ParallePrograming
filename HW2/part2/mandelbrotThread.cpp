...
void workerThreadStart(WorkerArgs *const args)
{
    ...
    /*unsigned int startRow, totalRows;
    
    if(args->height % args->numThreads != 0)
        totalRows = args->height / args->numThreads + 1;
    else
        totalRows = args->height / args->numThreads;
    
    startRow = args->threadId * totalRows;

    double startTime = CycleTimer::currentSeconds();


    if( args->threadId == args->numThreads-1){
        mandelbrotSerial(args->x0,args->y0,args->x1,args->y1,args->width,args->height,startRow,args->height-startRow,args->maxIterations,args->output);
    ...
    for(i = args->threadId ; i < args->height ; i = i + args->numThreads)
    {
